...
# Version: 1.0.
#
# Usage: merge-ret.pl [-v] <bl_dir> <save>\n";
...
my $usage = "merge-ret.pl [-v] <bl_dir> <save>\n";
my $verbose = 0;
my $rp = 0;
...
sub load_bl_ret_list(){
  ...
  for my $file (sort {$a cmp $b} @files){
    ...
    load_ret_file($ret_file, $BL_RANK_THRED);
  }
}

sub load_ret_file(){
  my $usage = "Usage: load_ret_file(\$ret_file, \$thred)\n";
  my $ret_file = shift;
  my $thred = shift;
